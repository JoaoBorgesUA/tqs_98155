...
public class Dip {
    ...
    private static final int STARSRANGE = 12;
    private static final int AMOUNTSTARS = 2;
    private static final int NUMBERSRANGE = 50;
    private static final int AMOUNTNUMBERS = 5;
    ...
    public Dip(int[] arrayOfNumbers, int[] arrayOfStarts) {
        ...
        if (AMOUNTNUMBERS == arrayOfNumbers.length && AMOUNTSTARS == arrayOfStarts.length) {
            for (int i : arrayOfStarts) {
                if(i>STARSRANGE){
                    throw new IllegalArgumentException("wrong range of number in stars selection");
                }
            }
